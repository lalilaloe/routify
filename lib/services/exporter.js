...
module.exports.exporter = async function exporter(params) {
    ...
    const { basepath = "", distDir, distFile, entryFile, convertToIndex, routifyDir } = params
    const entrypoint = `${distDir}/${entryFile}`

    const urls = require(resolve(routifyDir, 'urlIndex.json'))
    const script = fs.readFileSync(winResolve(distDir, 'build', distFile), 'utf8')
